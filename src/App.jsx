...
// 이름 체크리스트 산출
function calcNameChecklist(n, yongsinEl, isTrueYongsin, selectedSurname, nameSurname, johuEl=null, isJohuUrgent=false, tongwanEl=null, elementScores=null){
  const checks=[];
  ...
  const yongsinTypeLabel = isJohuUrgent && johuEl ? "조후용신" : yongsinEl ? "억부용신" : johuEl ? "조후용신" : tongwanEl ? "통관용신" : "보완오행";
  const primaryLabel = `${yongsinTypeLabel} ${EL_KR[primaryEl]||primaryEl}`;
  ...
  const soundDetail = soundMatchPrimary
    ? `한글 발음에 ${primaryLabel}의 기운을 담았습니다`
    : soundMatchSecondary
    ? `한글 발음에 보조용신 ${EL_KR[secondaryEl]||secondaryEl}의 기운을 담았습니다`
    : `한글 발음이 용신 오행과 다릅니다`;
  ...
  const charDetail = charMatchPrimary
    ? `한자에 ${primaryLabel}의 기운을 담았습니다`
    : charMatchSecondary
    ? `한자에 보조용신 ${EL_KR[secondaryEl]||secondaryEl}의 기운을 담았습니다`
    : charEl1||charEl2
...
function TaekIlSimulator(){
  ...
  function resolveNameYongsin(yongsin, tcpaBase, elementScores){
    const isJohuUrgent=Math.abs(tcpaBase.sBase)>=6;
    // 1. 조후 긴급
    if(isJohuUrgent && yongsin.johu?.primary){
      const type=tcpaBase.sBase<=-6?"차가운 기운을 따뜻하게 해주는":"뜨거운 기운을 식혀주는";
      return{primaryEl:yongsin.johu.primary, label:`조후용신 ${yongsin.johu.primary}`, desc:type, yongsinText:`조후 ${yongsin.johu.primary}(긴급) > 억부 ${yongsin.eobbu?.primary||""}`};
    }
    // 2. 억부
    ...
    if(yongsin.johu?.primary){
      return{primaryEl:yongsin.johu.primary, label:`조후용신 ${yongsin.johu.primary}`, desc:"기후의 균형을 잡아주는", yongsinText:`조후 ${yongsin.johu.primary}`};
    }
    // 4. 통관
    if(yongsin.tongwan?.primary){
      return{primaryEl:yongsin.tongwan.primary, label:`통관용신 ${yongsin.tongwan.primary}`, desc:"오행의 흐름을 통하게 하는", yongsinText:`통관 ${yongsin.tongwan.primary}`};
    }
    // 5. 부족 오행 (elementScores 최솟값)
    const total=Object.values(elementScores).reduce((a,b)=>a+b,0)||1;
  ...
  async function generateNames(saju, excludeNames=[]){
    ...
    try{
      ...
      const {primaryEl,yongsinText}=resolveNameYongsin(yongsin,tcpaBase,elementScores);
                      ...
                      {nameResult&&nameSaju===saju&&(
                        <div style={{display:"flex",flexDirection:"column",gap:8,animation:"slideUp 0.2s ease"}}>
                          {nameResult.map((n,ni)=>{
                            ...
                            const checks=calcNameChecklist(n,ys.primary,ys.isTrueYongsin,selectedSurname,nameSurname,ys.johu?.primary,isJohuUrgentCheck,ys.tongwan?.primary,calcStrengthDetail(saju.pillars).elementScores);
                            return(
